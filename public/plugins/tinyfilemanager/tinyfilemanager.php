...
$root_path = $_SERVER['DOCUMENT_ROOT'] . "/boavista/public/uploads/";
$root_url = 'boaVista/public/uploads';
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY) : ?>
                            ...
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                            </a>
                            <a title="<?php echo lng('CopyTo') ?>..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">
                                <i class="fa fa-files-o" aria-hidden="true"></i>
                            </a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank">
            ...
            ?>
                <tr>
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        <div class="filename">
                            ...
                            if (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp'))) {
                                echo '<a href="' . $filelink . '" title="' . $f . '"><i class="' . $img . '"></i> ' . fm_convert_win(fm_enc($f)) . '</a>';
                            } else {
                                echo '<a href="' . $filelink . '" title="' . $f . '"><i class="' . $img . '"></i> ' . fm_convert_win(fm_enc($f)) . '</a>';
                            }
                    ...
                    <td class="inline-actions">
                        ...
                        <?php if (!FM_READONLY) : ?>
                            <a title="Delete" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1209, 'Delete File','<?php echo urlencode($f); ?>', this.href);"> <i class="fa fa-trash-o"></i></a>
                            <a title="Rename" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
                            <a title="CopyTo" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <a title="DirectLink" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
                        <a title="Download" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1211, 'Download','<?php echo urlencode($f); ?>', this.href);"><i class="fa fa-download"></i></a>
                    </td>
                </tr>
            <?php
                flush();
...
<script>
    function selectFile(filePath) {
        // Pega o campo de texto pelo id e preenche com o caminho do arquivo selecionado
        document.getElementById('campoDeTexto').value = filePath;
    }
</script>
...
function fm_get_display_path($file_path)
{
    ...
    switch ($path_display_mode) {
        case 'relative':
            ...
            return array(
                'label' => 'Path',
                'path' => fm_enc(fm_convert_win('uploads/' . ltrim($relative_path, '/')))
            );
        case 'host':
            $relative_path = str_replace($root_path, '', $file_path);
            return array(
...
<script>
    function selectFile(fileUrl) {
        window.parent.selectFile(fileUrl);
    }

    // Adicione esta função para capturar cliques nos itens e acionar a seleção do arquivo
